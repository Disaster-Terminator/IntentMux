...
usage() {
  cat <<'EOF'
Usage: scripts/deploy_local_intentmux.sh [options]

Manually deploy the local IntentMux sidecar with pre/post verification.

Options:
  --dry-run              Print planned commands without running them.
  --allow-dirty          Allow deployment from a dirty git worktree.
...
if [[ -n "$(git status --short)" ]]; then
  ...
  if ((allow_dirty == 0)); then
    echo "refusing to deploy from a dirty worktree; pass --allow-dirty to override" >&2
    exit 1
  fi
else
...
if ((dry_run)); then
  ...
  log "deployment checks completed for commit $current_commit"
fi
