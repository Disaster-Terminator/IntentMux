...
Build a lightweight Chinese routing sidecar in `/path/to/gateway/gateway-semantic-router` and keep the existing LiteLLM Docker gateway on `127.0.0.1:4000` unchanged.
...
- The router repository is separate from `/path/to/gateway/litellm`; the LiteLLM mount directory and its secrets are not part of this git repository.
