...
def test_chat_completion_rewrites_smart_router_before_forwarding():
    ...
    app = create_app(
        router=FakeRouter(
            RoutingDecision(
                ...
                "hard_rule:线上",
                rewrite=True,
                route_id="strong route",
                policy_id="hard_rule/strict",
    ...
    assert response.headers["x-router-route-id"] == "strong%20route"
    assert response.headers["x-router-policy-id"] == "hard_rule%2Fstrict"
